...
def train(
    ...
    training_args = {
            ...
            'encode_rnn_hidden': encode_rnn_hidden or encode_all,
    ...
    for epoch in range(epoches):
        ...
        if validation:
            with torch.no_grad():
                ...
                my_model.train()
            progress_msg = 'epoch:%3d, loss:%.4f, valid:%.4f'\
                    % (epoch, total_loss, total_valid_loss)
            log_msg = '%d,%.4f,%.4f\n' % \
                    (epoch, total_loss, total_valid_loss)
        else:
            progress_msg = 'epoch:%3d, loss:%.4f'\
                    % (epoch, total_loss)
            log_msg = '%d,%.4f\n' % (epoch, total_loss)
        print(progress_msg)
        ...
        if validation:
            if epoch > 100 and total_valid_loss < min_valid_loss:
                min_valid_loss = total_valid_loss
                model_save_path = os.path.join(model_path, 'models_e%03d_vl%.4f.pt' % (epoch+1, total_valid_loss))
...
def main():
    ...
    train(
            ...
            encode_rnn_hidden=args.encode_rnn_hidden,
            encode_all=args.encode_all,
            encode_dim=args.encode_dim,
            common_space_dim=args.common_space_dim,
