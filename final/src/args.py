...
def get_args(train=True):
    ...
    if train:
        ...
        parser.add_argument('--encode_rnn_hidden',
                default=False,
                action='store_true',
                help='When encoding, use mean of all RNN\
                        hidden states or not.')
        parser.add_argument('--encode_all',
                default=False,
                action='store_true',
                help='When encoding, use mean of all RNN\
                        hidden states and mean of all input\
                        features or not. Overrides --encode_rnn_hidden.')
        parser.add_argument('--encode_dim',
                default=1024,
                type=int,
                help='Output dimension of encoding, available\
                        only when --encode_all is set.')
        parser.add_argument('--common_space_dim',
                default=128,
                type=int,
                help='Output dimension of common space.')
        parser.add_argument('--dist_fn',
                default='L2',
                type=str,
                help='Distance function used when calculating\
                        loss. Available choices: "L2", "cossim".')
    else:
        ...
        parser.add_argument('--models',
                ...
                help='When ensemble is True, this argument\
                        is required.')
        parser.add_argument('--args_filenames',
                nargs='*',
                help='When ensemble is True, this argument\
                        is required.')
